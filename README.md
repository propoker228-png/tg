...
Пункт **15) Тест скорости интернета** измеряет пропускную способность канала сервера (как Speedtest.net). Доступен **без установленного прокси и домена**. При первом запуске предлагается установить **Ookla Speedtest CLI**; если найден устаревший `speedtest-cli` (Python), скрипт предложит удалить его и поставить Ookla. При отказе или сбое используется упрощённый режим (**curl** download + **Cloudflare** upload + ping). Тест расходует интернет-трафик.
