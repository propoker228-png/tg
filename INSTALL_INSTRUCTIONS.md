...
- `13` - шейпинг трафика (Mbit/s на IP)
- `14` - лимит устройств (общая ссылка; приближение, мин. TCP 5)
- `15` - тест скорости интернета (Ookla или curl/ping; без домена)
- `0` - выход
...
Затем выберите пункт `11`. Откроется подменю:

- `1` — удалить стек (секрет `/root/telemt-secret.txt` и сертификаты Let's Encrypt **сохраняются**)
- `2` — полное удаление: секрет и сертификаты удаляются; для подтверждения нужно ввести `DELETE`
- `0` — назад
...
`--purge` можно использовать только вместе с `--uninstall`.

### Обычное удаление (`--uninstall` или меню `11 → 1`)
...
### Полное удаление (`--uninstall --purge` или меню `11 → 2`)

Дополнительно к обычному удалению:

- удаляется `/root/telemt-secret.txt`
- выполняется `certbot delete` для домена (best effort)
- удаляются `/opt/telemt`, `/opt/telemt-panel`, stub-сайты и файлы кластера
